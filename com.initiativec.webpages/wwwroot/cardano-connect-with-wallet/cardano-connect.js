...
class CardanoConnectWallet {
  constructor(
    ...
    this.wallet.addEventListener(
      ...
      this.wallet.addEventListener('usedAddresses', (usedAddresses) => {
  ...
  updateDropdownMenu() {
    ...
    const getDefaultButtonTitle = () => {
      if (this.showAccountBalance) {
        ...
        return formatStakeAddress(this.usedAddresses);
      };

      function formatStakeAddress(stakeAddress) {
          if (stakeAddress.length <= 20) {
              return stakeAddress;
          }
          const firstTen = stakeAddress.slice(0, 10);
          const lastTen = stakeAddress.slice(-10);
